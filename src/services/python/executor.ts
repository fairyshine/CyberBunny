...
export class PythonExecutor {
  ...
  private async syncFromDB(): Promise<void> {
    if (!this.pyodide) return;
    const entries = await this.fileSystem.readdir('/workspace');
  ...
  private async syncToDB(): Promise<void> {
    if (!this.pyodide) return;
    try {
      const entries = this.pyodide.FS.readdir('/workspace');
      for (const name of entries) {
        if (name === '.' || name === '..') continue;
        const pyPath = `/workspace/${name}`;
        ...
        if ((stat.mode & 0o170000) === 0o100000) {
          ...
          await this.fileSystem.writeFile(`workspace/${name}`, blob);
        }
      }
    } catch (e) {
